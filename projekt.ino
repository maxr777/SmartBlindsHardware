      ...
      case GET_SETTINGS:
        {
          ...
          doc["openLux"] = String(settings.openLux);
          doc["closeLux"] = String(settings.closeLux);
          doc["openMode"] = settings.openMode;
          doc["closeMode"] = settings.closeMode;

          String settingsJson;
          serializeJson(doc, settingsJson);
          SerialBT.println(settingsJson);
          break;
        }
...
void checkSchedule() {
  ...
  if (settings.openMode == "TIME" && now.hour() == openHour && now.minute() == openMinute) {
    openBlinds();
  }

  if (settings.closeMode == "TIME" && now.hour() == closeHour && now.minute() == closeMinute) {
    closeBlinds();
  }
}
